...
- R8. Named peers use the same per-provider candidate chain as the review skills, but ce-pov resolves and sanctions the concrete provider route and every intermediary before giving that process project content or repository access. If a dispatched route fails and the next candidate would change the provider or intermediary (for example, Grok via `grok-cli` falling through to `grok-cursor`), the worker returns the failure; the host discloses and sanctions the new actual route before retrying. A named peer that cannot run is reported, never silently dropped.
...
- Reuses the merged detached-peer infrastructure and its provider set (codex / claude / grok / composer, with the `grok-cli` → `grok-cursor` fallback). No new runner lifecycle or implementation is designed; ce-pov receives the required byte-identical runner copy and a POV-specific worker adapter.
...
**Product Contract preservation:** changed R3/R15 to distinguish ce-pov's read-only POV contract from an explicitly pre-authorized downstream handoff; expanded R5/R8/R9/R11/R13/R17/R21 and added R24-R26 plus AE12-AE17 for intent-aware shorthand, pre-sanctioned actual routes, repository-root read access with uniform narrowing, snapshot-consistent rounds, coordinator-verified evidence deltas, and explicit movement states. The user-facing behavior is session-settled; the route-sanction, payload, and repository-identity clauses close implementation contradictions discovered during plan review. The earlier oracle/offer cap, correction-cost gate, and all other Product Contract text remain preserved.
...
- **Requirements:** R8, R9, R10, R13, R16, R17, R21 (mechanics), R23 (adapters), R25, R26.
...
- **Requirements:** R23–R26 (falsifiable protocol, read boundary, and repository identity), plus the specific tokens from U1–U4.
...
- **Requirements:** R3, R21, R24, R25, R22 (docs must not promise removed behavior), repo plugin-maintenance convention.
- **Dependencies:** U1–U4.
- **Files:** `docs/skills/ce-pov.md`, `docs/skills/README.md` (catalog row), `README.md` (inventory row).
- **Approach:** Update the ce-pov docs page: purpose; three subject shapes; offer-first, named, and oracle panels; the two short conversational shorthand examples; repository-root read-only default and narrowed-filter disclosure; coordinator verification and movement reporting; honest recommendation; and the analysis-only versus pre-authorized handoff boundary. Keep when-to-use versus ce-doc-review and chain position unchanged. Update both inventory rows without hand-bumping versions or counts.
...
- **Test scenarios:** the seventeen cases and their R/AE mappings are enumerated in the Approach above; each case must produce its mapped behavior on both hosts.
...
|---|---|---|
| Full test suite | `bun test` | Pre-sanctioned actual routes, repository-grounded read-only safety, normalized uniform narrowing, snapshot-consistent rounds, no-truncation degradation, scratch cleanup, movement/schema gates, three-way byte-parity, contract guards, and no regression |
...
| Behavioral evals | skill-creator fixture pack (U7) on Claude Code and Codex | Original ten behaviors plus conversational shorthand, ambiguity handling, exact named participants, restricted shared read scope, classified evidence deltas, explicit movement, and mutation/handoff authority |
...
- U7's seventeen eval cases pass (or their fixes are applied and re-run) on both Claude Code and Codex.
