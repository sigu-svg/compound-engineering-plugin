...
**Approach:** Add a `### Grok Build CLI` section under `## Install` (mirroring `### Antigravity CLI (\`agy\`)` and `### Kimi Code CLI`) covering both `grok plugin install sigu-svg/compound-engineering-plugin` and `grok plugin marketplace add sigu-svg/compound-engineering-plugin`. Document the xai-org submission as a periodic follow-up: fork `xai-org/plugin-marketplace`, add one **remote-source** entry (our repo URL + full SHA from `git ls-remote`) to their `.grok-plugin/marketplace.json`, run their `generate-plugin-index.py` + `validate-catalog.py`, open the PR. State the SHA-bump-per-promotion cost (KTD5) so future maintainers know direct-install/self-hosted are the auto-current channels.
