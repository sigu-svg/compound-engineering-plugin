...
| E17 fallback after terminal | A `prefer` attempt has authoritatively failed before any canonical apply | Claim fallback exactly once, disclose the terminal failure, then run native; after commit and verification, record `complete-fallback` with the accepted HEAD and evidence digest so repeated resume neither restarts nor rediscovers it. |
