...
**Operational-question gate.** When the request is a *concept by inference* whose text reads as diagnosing or operating current behavior ("why is X doing Y", "is X configured right", "what's the trade-off between A and B here") rather than learning a subject, and it carries no build signal — no `diff:`/`since:` token, no wording that plainly asks to learn or build ("teach me how X works", "explain this change so it sticks") — answer it directly and offer the explainer as a one-line option ("Want a durable explainer on this instead?") rather than building unprompted; create the run directory and profile the repo only once the user accepts.
