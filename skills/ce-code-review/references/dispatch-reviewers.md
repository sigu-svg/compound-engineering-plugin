...
**Restricted-orchestrator rule (overrides inheritance).** Some installations run the orchestrator on a model licensed for orchestration only, which their own instructions declare must never execute as a subagent. When the session model is such a model, never dispatch any sub-agent by inheritance: give every dispatch an explicit `model` override — the installation's strongest allowed subagent tier for judgment-heavy roles, its mid-tier for mechanical roles (the installation's instructions name the concrete models). If the harness cannot apply an explicit override, do the work inline in the orchestrator instead of dispatching (fail closed): a skipped dispatch costs a little quality; a silently inherited restricted model burns the session budget this rule exists to protect.
