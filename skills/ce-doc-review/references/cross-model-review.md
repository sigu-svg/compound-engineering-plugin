...
Each call is a CLI shell-out, not a subagent, so it doesn't consume the subagent concurrency budget. **Resolve the peer provider once per document review** (after host attestation + preference resolution), then launch one call per activated trio lens with that **same** resolved provider **front-loaded ahead of the remaining candidate order** (e.g. candidates=`codex,claude,grok,composer` when you resolve to codex) so every lens shares one peer — while the trailing order preserves the cross-provider fallback if the resolved provider turns out installed-but-unauthed or throttled. (Passing the provider *alone* also pins one peer, but forfeits that fallback and skips the whole pass when the top choice is unusable.) Launch those calls as background shell processes in the same dispatch wave as the in-process persona reviewers so runtime overlaps, then **await every background script to exit before synthesis** — do not orphan them by exiting the launcher shell early (orphaned launches historically left un-normalized peer JSON). The script itself ignores SIGHUP and only publishes a fold-in `.json` after normalize.
...
The script embeds the **full document content** into the peer prompt and sends it to an external model provider (OpenAI, Anthropic, xAI, or Cursor, depending on the resolved peer). This is a wider egress than a diff-only review. `CROSS_MODEL_PEERS` restricts which providers may receive content. The peer runs strictly read-only and **tool-less**, from an empty scratch run-dir, with no project context — every route denies file reads, web, MCP, and subagents where the CLI supports it (codex `-s read-only -C <scratch>`; claude `--bare --tools ""` — all built-in tools disabled and no CLAUDE.md/MCP/hook auto-discovery — run from the scratch dir; grok denies `Read`/`Edit`/`Write`/`Bash`/`Task`/web/`mcp__*` with `--cwd <scratch>`), and the cursor-agent routes run `--mode ask --sandbox enabled --workspace <scratch>`. Impact is bounded to disclosure, not repo mutation — and a document from an untrusted author is a prompt-injection surface contained by the read-only, tool-less posture to disclosure-to-self.
